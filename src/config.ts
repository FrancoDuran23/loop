...
//   root selects.
// - DATABASE_URL: defaults to the docker-compose.yml Postgres service's own
//   credentials/port (Phase 5). This default is what makes the spec §18
//   zero-config clean-machine bootstrap work: `docker compose up -d && npm
//   i && npm run db:migrate && npm run pipeline -- ...` must produce ranked
//   deals with NO environment variables set at all, so DATABASE_URL cannot
//   be a required-with-no-default value. Still overridable via a real env
//   var for any non-default Postgres target (staging, CI, etc).
